...
features, not engine. (The `maps` feature owns the `WWMap` entity + CRUD and *depends on* the
...
- **The usecase is the only entry point.** Routers and other modules call a module *through its
  usecase methods* — never reach into its `dbo`/`validators` directly.
...
- Build on the layered base procedures in `src/server/trpc/trpc-setup.ts`:
  ...
  re-implement auth/match/turn checks in a procedure. Auth must be *enforced* by the procedure, not
  merely documented.
