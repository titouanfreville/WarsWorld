...
export class Vision {
  ...
  // Keyed by the packed tile index (y*width + x), NOT by the Position array reference. Callers pass a
  // fresh `unit.data.position` array when a property is captured, so a reference-keyed Set could never
  // find it to delete — the stale entry then re-granted the FORMER owner vision of the captured
  // property on the next `recalculateVision()` (i.e. every passTurn). The index is a tile's canonical
  // identity, so add/remove/recalc all agree regardless of which array instance is passed.
  private ownedProperties: Map<number, Position>;
  ...
  constructor(team: TeamWrapper) {
    ...
    this.ownedProperties = new Map<number, Position>();
  ...
  /** The packed array index of a tile — its canonical identity, independent of the Position instance. */
  private tileIndex(position: Position): number {
    return position[1] * this.mapWidth + position[0];
  }
  ...
  addOwnedProperty(position: Position) {
    const index = this.tileIndex(position);

    if (this.ownedProperties.has(index)) {
      return; // already counted — don't double-increment the vision counter
    }

    this.ownedProperties.set(index, position);
  ...
  removeOwnedProperty(position: Position) {
    ...
    if (!this.ownedProperties.has(index)) {
      return; // not owned here (or already removed) — don't underflow the vision counter
    }

    this.ownedProperties.delete(index);
