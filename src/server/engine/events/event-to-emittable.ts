import type { Path, Position } from "server/core/schemas/position";
import {
  addDirection,
  getFinalPositionSafe,
...
import { maskUnitForViewer, TeamWrapper } from "server/engine/entities/team";
import { createEmittableAttackEvent } from "server/engine/events/handlers/attack/attackEventToEmittable";
  ...
  switch (event.type) {
    case "move": {
      ...
      return teamsWithSpectator.map((team) => {
        ...
        // A unit appearing out of fog is masked for the receiving team (maskUnitForViewer): an enemy
        // never learns its transport cargo, nor a Sonja unit's true HP/fuel/ammo, from the reveal
        // payload — same rule the board (`match.full`) applies.
        const result: EmittableEvent = {
          ...
          appearingUnit:
            shownPath.length == 0 || team.isPositionVisible(event.path[0])
              ? undefined
              : maskUnitForViewer(match.getUnitOrThrow(getFinalPositionSafe(event.path)), team),
        };
