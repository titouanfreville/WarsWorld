import type { LuckRoll } from "server/core/schemas/co";
import { getDistance } from "server/core/schemas/position";
import type { UnitWrapper } from "server/engine/entities/unit";
import type { CombatProps } from "server/engine/rules/co-hooks";
import { getBaseDamage } from "server/engine/constants/base-damage";
...
export const calculateDamage = (
  ...
  // Sandstorm (AWDS weather): every attacking unit loses 30% firepower for as long as it lasts.
  // Applies whenever the current weather is sandstorm, regardless of game version.
  if (attacker.match.getCurrentWeather() === "sandstorm") {
    attackModifier -= 30;
  }
